...
def token_auth_required(func):
    """Separate token authentication decorator - COMPLETELY INDEPENDENT from authorization"""
    @wraps(func)
    async def wrapper(client, message):
        user_id = message.from_user.id
        ...
        is_verified = await token_manager.check_token_verification(user_id)
...
def authorized_only(func):
    """Main authorization decorator - SEPARATE from token system"""
    @wraps(func)
    async def wrapper(client, message):
        user_id = message.from_user.id
        ...
        # Step 4: Check if bot is in restricted mode
        if BOT_AUTHORIZED:
        ...
        # Step 5: Public mode - allow all users
...
class TokenManager:
        ...
        if user_id in self.global_tokens:
            ...
            if token_data["expires_at"] > time.time():
                if token_data["token"] == token:
                    return True
        return False
    async def verify_user_token(self, user_id: int, token: str) -> bool:
        """Verify user token - ONLY when they actually click the start link"""
        logger.info(f"🔐 Verifying token for user {user_id}: {token}")
        
        # Check pending tokens first (recently generated but not verified)
        if hasattr(self, 'pending_tokens') and user_id in self.pending_tokens:
            pending_data = self.pending_tokens[user_id]
            if (pending_data["token"] == token and 
                pending_data["expires_at"] > time.time() and
                ...
                # Remove from pending
                del self.pending_tokens[user_id]
                
                # Also update memory
                self.global_tokens[user_id] = {
                    "token": token,
                    "expires_at": pending_data["expires_at"],
                    "verified": True
                }
        ...
        if tokens_db is not None:
            ...
            if token_data:
                ...
                if (token_data.get("verified", False) and 
                    token_data.get("expires_at", 0) > time.time()):
        ...
        if user_id in self.global_tokens:
            token_data = self.global_tokens[user_id]
            if token_data.get("verified", False) and token_data["expires_at"] > time.time():
                if token_data["token"] == token:
    ...
    async def verify_token_db(self, user_id: int, token: str = None):
        ...
        return False
    async def check_token_verification(self, user_id: int):
        """Check if user is token verified - ONLY checks verified tokens"""
...
class EnhancedTaskQueue:
    ...
    async def process_queue(self):
        """Process queue with proper semaphore limiting - FIXED VERSION"""
        while True:
            try:
                # Use the global semaphore to limit concurrent tasks
                async with semaphore:
                    ...
                    # ✅ Safety check for corrupted/cleared queue items
                    if not isinstance(item, (tuple, list)) or len(item) < 4:
                        logger.warning(f"⚠️ Skipping invalid queue item: {item}")
                        continue

                    priority, task_id, task_callable, task_data = item
                    chat_id = task_data['chat_id']
                    
                    # 🎯 CRITICAL FIX: Check if user already has a running task
                    async with self.task_lock:
                        if chat_id in self.active_tasks:
                            existing_task = self.active_tasks[chat_id]
                            existing_status = existing_task.get('status', 'unknown')
                    ...
                    self.active_count += 1  # Increment active count
                    ...
                    logger.info(f"📊 Active tasks: {self.active_count}/{self.max_concurrent}")
                    ...
                    # ✅ Safe task_done() handling
                        try:
                            self.queue.task_done()
                            logger.info(f"✅ Task {task_id} completed and queue task_done() called safely")
                        except ValueError:
                            logger.warning(f"⚠️ task_done() called too many times for task {task_id} — queue already cleared or inconsistent")
...
class VideoEncoder:
    ...
    def _build_ffmpeg_command(
        ...
        quality_settings = {
            '360p': {
                'height': 360,
                'crf': 30 if codec == 'h264' else (32 if codec == 'h265' else 33),
                'preset': 'medium',
            ...
            '480p': {
                'height': 480,
                'crf': 29 if codec == 'h264' else (30 if codec == 'h265' else 31),
                'preset': 'medium',
                'pix_fmt': validated_pixel_format,
                'profile': validated_profile,
                'max_bitrate': '1500k',
            ...
            '720p': {
                'height': 720,
                'crf': 28 if codec == 'h264' else (29 if codec == 'h265' else 30),
                'preset': 'medium',
                'pix_fmt': validated_pixel_format,
                'profile': validated_profile,
                'max_bitrate': '2500k',
            ...
            '1080p': {
                'height': 1080,
                'crf': 27 if codec == 'h264' else (28 if codec == 'h265' else 30),
            ...
            '4k': {
                'height': 2160,
                'crf': 25 if codec == 'h264' else (27 if codec == 'h265' else 29),
                'preset': 'slow' if codec in ['h264', 'h265'] else 'fast',
                'pix_fmt': validated_pixel_format,
                'profile': validated_profile,
                'max_bitrate': '12000k',
...
async def process_batch(
    ...
    try:
        ...
        valid_qualities = ["360p", "480p", "720p", "1080p", "4k", "noencode"]
...
async def start_handler(client: Client, message: Message):
    """Handle start command with PROPER token verification - FIXED VERSION"""
    try:
        user_id = message.from_user.id
        command_args = message.text.split()
        
        # Handle token verification from start command
        if len(command_args) > 1 and command_args[1] != "start":
            token = command_args[1]
            
            logger.info(f"🔐 Token verification attempt for user {user_id}: {token}")
            
            # 🎯 ACTUALLY VERIFY THE TOKEN (this will mark it as verified)
            is_verified = await token_manager.verify_user_token(user_id, token)
            
            if is_verified:
                # ✅ Token valid and now marked as verified
                verification_msg = await message.reply(
                    "✅ **Token Verified Successfully!**\n\n"
                    f"⏰ **{TOKEN_DURATION_HOURS}-Hour Access Activated**\n"
                    "• Full normal access granted\n"
                    "• Happy encoding! 🎬\n\n"
                    "Use /magnet or /compress to start encoding."
                )
                await asyncio.sleep(5)
                try:
                ...
                await message.reply(
                    ...
                    "Please use /magnet or /compress to get a new token."
                )
                return
        
        # Normal start command - check if user needs token
        if (TOKEN_ENABLED and 
            user_id not in SUDO_USERS and 
            not await is_premium_user(user_id) and
            not await token_manager.check_token_verification(user_id)):
            
            logger.info(f"🔐 User {user_id} needs token, sending prompt")
            await message.reply(
                "🔐 **Token Verification Required**\n\n"
                "You need to verify with a token to use this bot.\n"
                "Please use /magnet or /compress command to get your access token."
            )
            return
        
        # User is authorized or has valid token, show main menu
        await show_main_menu(client, message)
...
async def debug_token_flow_handler(client: Client, message: Message):
    ...
    debug_info.append(f"✅ Token Verified: `{await token_manager.check_token_verification(user_id)}`")
